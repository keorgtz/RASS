  ...
  tui: async (api, _options, _meta) => {
    ...
    const showModeProfileDialog = (dialog) => {
      ...
      dialog.replace(
        () => api.ui.DialogSelect({
          ...
          placeholder: 'Select a ModeProfile, create new, or assign to agent...',
          options,
          current: currentMp,
          onSelect: (option) => {
    ...
    const showSetupDialog = (dialog) => {
      ...
      const options = [
        ...
        {
          title: 'Switch to RyouGo',
          value: 'ryougo',
          description: 'Switch to RyouGo ModeProfile (full pipeline with all subagents)',
        },
        {
            ...
            onSelect: (option) => {
              switch (option.value) {
                ...
                case 'ryougo': {
                  ...
                  api.ui.toast({
                    variant: 'success',
                    title: 'Switched to RyouGo',
                    message: 'ModeProfile: RyouGo — Full pipeline with per-phase model routing. Agent models synchronized.',
                  });
                } catch (err) {
                  dialog.clear();
                  api.ui.toast({ variant: 'error', title: 'Error', message: err.message });
