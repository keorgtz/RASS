# Design System — SHEndevour Web
> Versión 1.0 · Fusión MetricsDashboard + ReprocesoDashboard
...
- **Fondo:** `#FFFFFF` (blanco, igual que titlebar y statusbar)
- **Borde derecho:** 1 px `--gray-line`
- Ítem activo: fondo `--em-bg` (#F0FDF4), borde izquierdo 3 px `--em` (#10B981), texto y ícono en `--em`
- Ítem hover: fondo `#F9FAFB`
- Ícono: 18 px, color `--gray-muted` inactive → `--em` active → `--gray-dark` hover
- Label: 13 px, Segoe UI 500, `--gray-muted` inactive → `--em` active (600) → `--gray-dark` hover
- Separadores de grupo: línea `--gray-line` + label de categoría 9 px uppercase `--gray-muted`
